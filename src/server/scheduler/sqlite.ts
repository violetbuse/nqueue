...
export class SchedulerSqlite extends Scheduler {
  ...
  override async find_cron_jobs_to_schedule(): Promise<string[]> {
    const cron_jobs = this.db
      ...
      .leftJoin(
        ...
        eq(sqlite_schema.cron_job.id, sqlite_schema.scheduled_job.cron_job_id),
      )
    ...
    return cron_jobs.map((job) => job.cron_id);
  }
}
